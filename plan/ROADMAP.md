...
- 2026-06-27 — _(este commit)_ — **Mercado vivo: preços oscilam + UI estilo bolsa.** Antes os preços
  só caíam (oferta monotônica em `box.opened`) e a UI era uma linha estática — não dava p/ ver
  flutuação. Agora o **worker** dá um tick periódico (a cada `recalc_interval_seconds`) com um
  "sentimento de mercado" (passeio aleatório com reversão à média, ~±25% em torno do valor-justo por
  escassez) → os preços **sobem e descem** ao vivo; publica `MARKET_UPDATED` no canal de **todas as
  salas** (mercado é global). O **frontend** guarda o histórico por item e o painel **Mercado** virou
  uma cotação: selo "● ao vivo", uma linha por item com **mini-gráfico (sparkline)**, preço animado a
  cada mudança e **seta de tendência ▲/▼ + variação** (verde sobe / vermelho cai). Verificado: WS 3/3
  (vários ticks, snapshots distintos, sobe E desce); screenshots desktop + mobile (390px) com
  sparklines/setas; `build` limpo, `py_compile` ok.
- 2026-06-27 — _(este commit)_ — **Troféu só p/ o vencedor do lote.** No `BOX_SOLD`, o 🏆 "Você
  arrematou!" (+ confete) fica só para quem venceu; os demais veem "<nome> arrematou" com o **martelo
  🔨** (a casa batendo o lote) + o preço — sem troféu. O carimbo "ARREMATADO" no palco continua p/
  todos. Verificado em navegador (3/3): vencedora vê 🏆; os outros veem 🔨 + quem venceu, sem troféu.
- 2026-06-27 — _(este commit)_ — **Feature: caixa também pode dar uma CARTA (prêmio extra).** Ao abrir
  uma caixa COM itens, há uma chance (por TIPO de baú, em `balance.yaml → box_card_chance`:
  5/12/22/35%) de vir TAMBÉM uma carta aleatória (pesos de `cards.weights`), que vai p/ a mão —
  **junto** dos itens, não no lugar. Mímico não dá carta. Sorteio pré-rolado no `startRound` (seed
  reproduzível) → `OpenResult.cardDropped`; o Leilão concede via novo RPC **`GrantCard`** da Carteira
  (grátis, respeita o `hand_max` — mão cheia = prêmio perdido); `OpenBoxReply.card` → gateway
  `OPEN_RESULT` → frontend mostra "Carta bônus! 🃏" e a mão cresce. Verificado: `mvn` 19 verde (+2
  testes: chance 100 → carta, sem chance → nenhuma); WS 4/4 (carta + itens juntos, mão +1). `tsc`/
  `build` limpos.
